...
```
>>> { :x | x / 2 }.nestWhileList(123456, isEven:/1)
...
```
>>> { :x | x.log }.nestWhileList(100, isPositive:/1)
...
```
>>> { :x | (x / 2).floor }.nestWhileList(20) { :x | x > 1 }
[20 10 5 2 1]
```
...
```
>>> { :x | x + 1 }.nestWhileList(899) { :x | x.isPrime.not }
...
```
>>> { :x | (x * 2) % 19 }.nestWhileList(2) { :x | x ~= 1 }
