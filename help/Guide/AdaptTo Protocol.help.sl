...
Each message transmission reduces a polymorphic variable to a monomorphic one,
because of the type dispatch inherent in message lookup.
...
This approach works well with methods that call a primitive and where the primitive returns immediately if the second type is immediately compatible and fails if it is not.

For instance,
when evaluating _p + q_ the system selects the appropriate `+` given the type of _p_.
The selected method will perform the addition if _q_ is of the same type as _p_,
