...
Test that a file exists:
	...
	system.systemCommand('test', ['-f', '/etc/passwd']).then { :result | (result::exitCode = 0).postLine }

Count the words in a file:

	system.systemCommand('wc', ['-w', '/etc/passwd']).then { :result | result::outputText.postLine }
