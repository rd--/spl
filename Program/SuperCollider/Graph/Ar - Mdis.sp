...
Voicer(1, 16) { :e |
	...
	let fund = GrainSin(1, gTrg, overlapA * (1.05 - e1.Power(1.1)) / f1, f1, p, -1, 512);
	let a = GrainSin(2, gTrg, overlapA * (1.05 - e1.Power(1.5)) / f1, f1 * formantA, p, -1, 512);
	let b = GrainSin(2, gTrg, overlapB * (1.05 - e1.Power(2.5)) / f1, f1 * formantB, p, -1, 512);
	let c1 = fund * (level * e1).Power(0.7);
	let c2 = a * (level * e1).Power(f2.LinLin(50, 1500, 1.1, 1.5));
	let c3 = b * (level * e1).Power(f2.LinLin(50, 1500, 1.5, 2.5));
